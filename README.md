...
- **完整的算法学习助手链路**：覆盖提问、检索、生成、保存会话的完整流程，不只是一个聊天页面。
- **语义检索更精准**：使用 `sentence-transformers + FAISS` 做本地知识库语义检索，比关键词匹配更懂“意思”。
- **支持流式输出**：回答可以边生成边展示，交互体验更自然。
- **具备兜底机制**：模型接口不可用时会自动切换到本地回答，保证项目可用性。
- **会话持久化**：聊天记录支持保存和恢复，刷新后还能继续查看历史对话。
- **兼顾工程稳定性**：从 JSON 迁移到 SQLite，并保留旧数据自动导入，便于长期维护。
- **适合简历展示**：技术栈清晰，功能完整，能直接演示 RAG / Agent 原型能力。
...
5. 当前会话会被保存到本地 SQLite 文件 `data/sessions_store.sqlite3`
6. 左侧列表会显示最近会话
